...
class PedestrainCsvAPI:
	def __init__(self):
		self.mypath="D:\\UNI\\data_engineering2\\heidelberg.csv"
		self.step = 1
	def csv_reader(self):
...
class Pdstrian_producer:
	...
	def send_csv_msg(self):
		...
		for row in pedestrain_instance.csv_reader():
			time.sleep(0.2)
			row2 = ''.join(chain(*row))
			date = pd.to_datetime(" ".join(row2.split(sep=" ")[2:4]), format='%Y-%m-%d %H:%M:%S')
			base = datetime(1979, 1, 1)
			ts = int((date - base).total_seconds())
			print(f"row : {row2},\n date :{ts}")
			self.producer.send(topic="pedestrian_csv", key=str(ts), value=row2)
...
# 	ts = int((date - base).total_seconds())
# 	print(f"row : {row2},\n date :{ts}")
# 	producer.send(topic="pedestrian_csv", key=str(ts), value=row2)
